...
const additionpokemon = [pokemon.find(pokemon => pokemon.type == 'water') , pokemon.find(pokemon => pokemon.type == 'fire') , pokemon.find(pokemon => pokemon.hp > 100)];

for(const pokemon of additionpokemon){
    if (pokemon){
...
for(let i = 0; i < game.party.length; i++){
    ...
    if (pokemon.Number == 1){
        game.party[i] = {pokemon, Name: "ivysaur" , Number: 2};
    }
    else if (pokemon.Number == 4){
        game.party[i] = { pokemon, Name: "charmeleon" , Number: 5};

    }

    else if (pokemon.Number == 7){
        game.party[i] = {pokemon, Name: "wartortle" , Number: 8};

    }

    else if (pokemon.Number == 25){
        game.party[i] = {pokemon, Name: "raichu" , Number: 26};
    }
